  ...
  #:endif
    ...
    if (this%isElecDyn) then
      ...
      if (this%isHybridXc) then
        ...
        if (input%ctrl%elecDynInp%tForces&
            & .and. input%ctrl%hybridXcInp%hybridXcAlg /= hybridXcAlgo%matrixBased) then
          call error("Forces for time propagation in combination with hybrid xc-functionals are&
              & currently only available for the matrix-based algorithm.")
        end if
        if (input%ctrl%elecDynInp%tIons&
            & .and. input%ctrl%hybridXcInp%hybridXcAlg /= hybridXcAlgo%matrixBased) then
          call error("Ion dynamics time propagation in combination with hybrid xc-functionals is&
              & currently only available for the matrix-based algorithm.")
        end if
      end if
