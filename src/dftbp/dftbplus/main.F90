...
contains
  ...
  subroutine processGeometry(this, env, iGeoStep, iLatGeoStep, tWriteRestart, tStopScc,&
    ...
    logical :: tConstrConverged, tConstrIterNotConverged = .false.
    ...
    REKS_SCC: if (allocated(this%reks)) then
      ...
      lpSCC: do iSccIter = 1, this%maxSccIter
        ...
        if (.not. tConstrConverged) then
          ! Remembers if at some point a micro-iteration cycle did not converge
          tConstrIterNotConverged = .true.
  ...
  #:endif
    ...
    if (allocated(this%elecConstrain)) then
      if (tConstrIterNotConverged) then
        call warning("Constraints did NOT converge at some point, maximal micro-iterations&
            & exceeded")
    ...
    if (this%tWriteDetailedOut .and. this%deltaDftb%nDeterminant() == 1) then
      call writeDetailedOut4(this%fdDetailedOut%unit, this%tSccCalc, allocated(this%elecConstrain),&
          & tConverged, .not. tConstrIterNotConverged, this%isXlbomd, this%isLinResp, this%isGeoOpt&
