...
module dftbp_mixer_broydenmixer
  ...
  use dftbp_dftb_sparse2dense, only : hermitianSquareMatrix
  ...
  type TBroydenMixerCmplx
    ...
    !> Charge difference in last iteration
    complex(dp), allocatable :: qDiffLast(:)

    !> Input charge in last iteration
    complex(dp), allocatable :: qInpLast(:)
...
contains
  ...
  !! norm of the charge difference vector. If the calculated weigth is outside of the
  !! [minWeight, maxWeight] region it is replaced with the appropriate boundary value.
  subroutine TBroydenMixerCmplx_init(this, mIter, mixParam, omega0, minWeight, maxWeight, weightFac)

    !> An initialized Broyden mixer on exit
    type(TBroydenMixerCmplx), intent(out) :: this
  ...
  end subroutine TBroydenMixerCmplx_init
  ...
  !> Makes the mixer ready for a new SCC cycle.
  subroutine TBroydenMixerCmplx_reset(this, nElem)

    !> Broyden mixer instance
    type(TBroydenMixerCmplx), intent(inout) :: this
    ...
    this%iIter = 0
    this%ww(:) = 0.0_dp
    this%aa(:,:) = (0.0_dp, 0.0_dp)

  end subroutine TBroydenMixerCmplx_reset
  ...
  !> Mixes charges according to the modified Broyden method.
  subroutine TBroydenMixerCmplx_mix(this, qInpResult, qDiff)
    ...
    call modifiedBroydenMixingCmplx(qInpResult, this%qInpLast, this%qDiffLast, this%aa, this%ww,&
        & this%iIter, qDiff, this%alpha, this%omega0, this%minWeight, this%maxWeight,&
        & this%weightFac, this%nElem, this%dF, this%uu)

  end subroutine TBroydenMixerCmplx_mix
  ...
  !> Does the real work for the Broyden mixer.
  subroutine modifiedBroydenMixingCmplx(qInpResult, qInpLast, qDiffLast, aa, ww, nn, qDiff, alpha,&
      & omega0, minWeight, maxWeight, weightFac, nElem, dF, uu)
    ...
    !> Input charge vector of the previous iterations
    complex(dp), intent(inout) :: qInpLast(:)

    !> Charge difference of the previous iteration
    complex(dp), intent(inout) :: qDiffLast(:)
    ...
    !> Prev. DFs
    complex(dp), intent(inout) :: dF(:,:)

    !> Prev. U vectors
    complex(dp), intent(inout) :: uu(:,:)

    complex(dp), allocatable, target :: cc(:)
    complex(dp), allocatable :: beta(:,:)

    ! Current DF or U-vector
    complex(dp), allocatable :: dF_uu(:)
  ...
  end subroutine modifiedBroydenMixingCmplx
  ...
  subroutine TBroydenMixerCmplx_getInverseJacobian(this, invJac)
    ...
    integer :: ii, jj, kk, mm, nn
    complex(dp), allocatable :: beta(:,:), zeta(:)
    ...
    call hermatinv(beta, errStatus, uplo='L')
    if (errStatus%hasError()) call error(errStatus%message)
    call hermitianSquareMatrix(beta)
    do ii = 1, mm
      do jj = 1, mm
        beta(ii, jj) = beta(ii, jj) * this%ww(ii) * this%ww(jj)
      end do
    ...
    do kk = 1, mm
      ...
      call ger(invJac, (1.0_dp, 1.0_dp), zeta, this%dF(:,kk))
    end do
