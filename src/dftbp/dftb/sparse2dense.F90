...
#:endif
  ...
  public :: getUnpackedOverlapStress_real!, getUnpackedOverlapStress_kpts
...
#:else
  ...
  subroutine getUnpackedOverlapStress_real(iCoordDist, iAtomPrime, skOverCont, orb, derivator,&
    ...
    !! Distance vector between atom iAtomPrime (central cell) and atom iAt2 (possibly in image cell)
    real(dp) :: distVect(3)
    ...
    do iNeigh = 1, nNeighbourCamSym(iAtomPrime)
      ...
      ! if (iAtomPrime == iAt2fold) cycle
      descAt2 = getDescriptor(iAt2fold, iSquare)
      distVect(:) = rCoords(:, iAtomPrime) - rCoords(:, iAt2)
      overPrime(:,:,:) = 0.0_dp
      call derivator%getFirstDeriv(overPrime, skOverCont, rCoords, symNeighbourList%species,&
            & iAtomPrime, iAt2, orb)
      do iCoord = 1, 3
        overSqrPrime(iCoord, descAt2(iStart):descAt2(iEnd), descAt1(iStart):descAt1(iEnd))&
            & = overSqrPrime(iCoord, descAt2(iStart):descAt2(iEnd), descAt1(iStart):descAt1(iEnd))&
            & + distVect(iCoordDist) * overPrime(1:descAt2(iNOrb), 1:descAt1(iNOrb), iCoord)
      end do
    end do
